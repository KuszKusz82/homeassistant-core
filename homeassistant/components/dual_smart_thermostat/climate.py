...
class DualSmartThermostat(ClimateEntity, RestoreEntity):
    ...
    def __init__(
        ...
        self._min_ceiling_temp = min_ceiling_temp
    ...
    @property
    def ceiling_temperature_limit(self):
        """Return the maximum floor temperature."""
        return self._min_ceiling_temp
    ...
    @property
    def _is_ceiling_cold(self):
        """If the ceiling temp is below limit."""
        if (
            (self.sensor_ceiling_entity_id is not None)
            and (self._min_ceiling_temp is not None)
            and (self._cur_ceiling_temp is not None)
        ):
            if self._cur_ceiling_temp <= self._min_ceiling_temp:
                return True
        return False
